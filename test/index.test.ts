...
describe("docker v2 bearer token-auth", () => {
  ...
  describe("decodeBase64Loose accepts standard and url-safe base64", () => {
    ...
    test("tolerates surrounding/embedded whitespace (env secrets, PEM wrapping)", () => {
      const s = btoa("user:pass");
      expect(decodeBase64Loose(`${s}\n`)).toBe("user:pass");
      expect(decodeBase64Loose(`  ${s}  `)).toBe("user:pass");
      // line-wrapped
      const long = btoa("x".repeat(60));
      const wrapped = long.replace(/(.{20})/g, "$1\n");
      expect(decodeBase64Loose(wrapped)).toBe("x".repeat(60));
    });
  ...
  describe("scopeAuthorizes gates the cross-repo mount source", () => {
    ...
    test("denies by default when there is no payload (no silent fail-open)", () => {
      expect(scopeAuthorizes(null, "any/repo", "pull")).toBe(false);
      expect(scopeAuthorizes(undefined, "any/repo", "pull")).toBe(false);
    });
